...
BINGMAP_KEY = "AvSKxRBCvGtIe-XFibJ8Cbh6WpRYiapi8MP_rUW1PX8uVEatSnodtvin2rlfkMcj"
...
def size_of_map(levelOfDetail):
    return int(256 << levelOfDetail)

def resolution_of_ground(latitude, levelOfDetail):
    latitude = clip(latitude, MIN_LATITUDE, MAX_LATITUDE)
    
    return math.cos(latitude * math.pi / 180) * 2 * math.pi * EARTH_RADIUS / size_of_map(levelOfDetail)

def scale_the_map(latitude, levelOfDetail, screenDpi):
    return resolution_of_ground(latitude, levelOfDetail) * screenDpi / 0.0254

def longlatToPixelXY(latitude, longitude, levelOfDetail):
    latitude = clip(latitude, MIN_LATITUDE, MAX_LATITUDE);
    ...
    mapsize = size_of_map(levelOfDetail);
    pixelX = int(clip(x * mapsize + 0.5, 0, mapsize - 1))
    pixelY = int(clip(y * mapsize + 0.5, 0, mapsize - 1))

    return pixelX, pixelY   

def pixelXYTolonglat(pixelX, pixelY, levelOfDetail):
    mapSize = size_of_map(levelOfDetail)
...
def latLongToTileXY(latitude, longitude, levelOfDetail):
    pixelX, pixelY = longlatToPixelXY(latitude, longitude, levelOfDetail)
...
def main():
    ...
    print("The longitude and latitude of the bounding box is (upper left point, lower right point)\n<%s, %s, %s, %s>" % (lat1, lon1, lat2, lon2))
