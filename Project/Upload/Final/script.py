...
def camera_config(camera_config_file):
...
def clustering(p_c_d):
    X = StandardScaler().fit_transform(p_c_d)
    clusters = DBSCAN(eps=.5, min_samples=10).fit_predict(X)
    df = pd.DataFrame([p_c_d['longitude'], p_c_d['latitude'], p_c_d['altitude'],clusters]).T
    df.columns = ['longitude', 'latitude', 'altitude','labels'];
    max_label = df.labels;
    max_cluster = df[df['labels']==max_label];
    print(max_cluster)
    ...
    c = [1]*len(np_chunk[:,0])
    ax.scatter(np_chunk[:,0], np_chunk[:,1], c=np_chunk[:,2], s=.1)
    plt.savefig("/content/Road-Boundary-Detection/output/dbscan.png")
...
def main(fuse_file, camera_config_file):
   ...
   x,y,z=gps2ecef(np.array(point_clouds['longitude']),np.array(point_clouds['latitude']),np.array(point_clouds['altitude']))
   ...
   delY=2  
   heightcar=3
   cam_lat, cam_lon, cam_alt, cam_qs, cam_qx, cam_qy, cam_qz=camera_config(camera_config_file)
   ...
   #do clustering
   dbscan_cp = point_clouds_cp.drop(columns="level")
   clustering(dbscan_cp)
   ...
   plt.savefig('/content/Road-Boundary-Detection/output/3dplot.png')
   ...
   plt.colorbar()
   plt.savefig('/content/Road-Boundary-Detection/output/2dplot.png')
   print('Saving figure...')
